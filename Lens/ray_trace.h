...
struct LensInterface 
{
	...
	float radius;// radius of shpere / plane

	vec3 n;		 // refractive indices (n0, n1, n2)
	float sa;	// nominal radius (from opt, axis)

	float d1;	// coating thickness = lambdaAr / 4 / n
	float flat; // is this iterface a plane ?
	float pos;
	float w;
};
...
vec3 reflect(vec3 i, vec3 n) { return i - n * 2.f * dot(i, n); }
...
float length_xy(vec3& v) { return sqrt(v.x * v.x + v.y * v.y); }
...
/// To simulate a quarter-wave coating optimized for wavelength
/// lambda0 under normal incidence, and given refractive indices n0(e.g air)
/// and n2(e.g glass), d1 and n1(thickness and refractive index of coating) would be chosen as
...
float FresnelAR(
	...
	// 2 transmission and 1 reflection
	...
	// Add up sines of idfferent phase and amplitude
...
) 
{
	...
	for (k = 0; k < LEN; k++, T += DELTA) {
		...
		if (PHASE == 0) { intersections1.push_back(i.pos); } 
		else if (PHASE == 1) 
		{
		...
		if (!F.flat) { r.tex.z = max(r.tex.z, length_xy(i.pos) / F.sa); }
		else if (T == AP_IDX) 
		{
